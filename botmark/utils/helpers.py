...
def get_models(models_dir):
    ...
    if not models_dir:
        return {"object": "list", "data": botmark_models}
    ...
    for suffix in suffixes:
        for f in models_dir.rglob(f"*{suffix}"):
            if f.is_file():
                try:
                ...
                botmark_models.append({
                    "id": str(relative_path).replace("\\", "/"),  # für Windows: Pfadtrennzeichen vereinheitlichen
                    "object": "model",
                    "created": created,
                    "owned_by": "botmark"
                })
...
def get_model(model_id, models_dir):
    ...
    for suffix in suffixes:
        # handle subfolders (e.g. subdir/deep_model)
        model_path = models_dir / (model_id + suffix)
            ...
            try:
                with open(model_path, "r", encoding="utf-8") as f:
                    if suffix == ".json":
                        ...
                        content = f.read()
                        return parser.parse_to_json(content)
            except Exception as e:
